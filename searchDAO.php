...
if (!$con) { //Checa a conexão
    ...
    if ($search === "") { //Se estiver sem pesquisa, ele chama a função que pega todas as informações do banco de dados
        searchController::getAll($con);
    } else { //Se não, ele filtra
        searchController::search($con, $search);
    }
}
class searchController
{
    public static function getAll($cone) //Função que pega todas as informações.
    {
    ...
    public static function search($cone, $search) //Função para filtrar.
    {
